...
async function autoGenerateDeptSchedule(dept, dates) {
  ...
  // 各ポジション50音ソート
  for (const posId of Object.keys(candidatesByPos)) {
    candidatesByPos[posId].sort((a, b) => a.localeCompare(b, 'ja'));
  }

  // ラウンドロビンで割当（各ポジションごとに独立したカウンタ）
  const posCounters = {};
  for (const pos of cfg.positions) {
    posCounters[pos.id] = 0;
  }
  ...
  for (const { date } of dates) {
    ...
    for (const pos of cfg.positions) {
      ...
      const idx = posCounters[pos.id] % candidates.length;
      result[`${dept}_${pos.id}_${ymd}`] = {
        dept, position: pos.id, date: ymd, assignee: candidates[idx],
      };
      posCounters[pos.id]++;
    }
  }

  // 同一日に同じ人が複数ポジションに入らないよう調整
  for (const { date } of dates) {
    const ymd = fmtYmd(date);
    const usedNames = new Set();
    const positionsToResolve = [];

    // 1パス: 候補者が少ないポジションから確定
    const sortedPositions = [...cfg.positions].sort((a, b) =>
      (candidatesByPos[a.id]?.length || 0) - (candidatesByPos[b.id]?.length || 0)
    );

    for (const pos of sortedPositions) {
      const key = `${dept}_${pos.id}_${ymd}`;
      const entry = result[key];
      if (!entry) continue;

      if (usedNames.has(entry.assignee)) {
        // 衝突 → 別の候補者を探す
        const candidates = candidatesByPos[pos.id];
        const alt = candidates.find(c => !usedNames.has(c));
        if (alt) {
          entry.assignee = alt;
        }
        // 見つからなければそのまま（警告で表示）
      }
      usedNames.add(entry.assignee);
    }
  }
