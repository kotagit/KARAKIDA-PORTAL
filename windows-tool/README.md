...
#### ⚠️ ダウンロード後、起動時に「アクセスできません」と出る場合

> `指定されたデバイス、パス、またはファイルにアクセスできません。これらの項目にアクセスするための適切なアクセス許可がない可能性があります。`

これは Windows がインターネット由来の未署名 EXE を自動ブロックしている状態（Mark-of-the-Web / SmartScreen）。
ウイルスではなく、以下のいずれかで解除できる。

**方法 A: エクスプローラーから解除（推奨）**

1. ZIP を**展開する前に**、ZIP ファイルを右クリック → **プロパティ**
2. 下部の **「セキュリティ: このファイルは他のコンピューターから取得したものです…」** にある **「許可する」** にチェック → OK
3. その後 ZIP を展開 → `MWB_Excel_Tool.exe` をダブルクリック

> 既に展開してしまった場合は、展開後の `.exe` を右クリック → プロパティから同じ操作。

**方法 B: PowerShell で一括解除**

ダウンロードフォルダを開いて Shift + 右クリック → 「PowerShell ウィンドウをここで開く」:

```powershell
Unblock-File .\MWB_Excel_Tool.exe
```

**方法 C: SmartScreen の青い画面が出た場合**
...
ブロックされる根本原因は EXE にコード署名が付いていないこと（社内配布ツールなので未署名）。一度解除すれば次回以降は警告は出ない。
